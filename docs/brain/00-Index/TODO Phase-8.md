...
- [x] **T8.2.2** Coach subscription billing — `receiving half done` · the webhook HANDLER that owns subscription state: status map (unknown fails CLOSED to canceled), tier↔price as DATA (028), out-of-order guard on `provider_event_at`, coach resolution by subscription → customer → Checkout metadata (verified against `users`, never trusted). `verify:subscription` 29/29 walks the whole chain — signed event → tier → seat cap follows, existing links untouched. **NOT done: creating the Checkout Session** (the outbound half), which needs live Stripe credentials this environment does not have
