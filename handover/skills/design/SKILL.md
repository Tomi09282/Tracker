...
| Tokens, specs, CSS vars | `design-system-tokens` | External skill |
...
Multi-platform social image design: HTML/CSS → screenshot export. Uses `ui-ux-pro-max`, `brand`, `design-system-tokens`, `chrome-devtools` skills.
...
4. **Design** — `/ckm:brand` → `/ckm:design-system-tokens` → randomly invoke `/ck:ui-ux-pro-max` OR `/ck:frontend-design`; HTML per idea × size
...
2. **Tokens** (design-system-tokens skill) → Create semantic token layers
...
**External sub-skills:** brand, design-system-tokens, ui-styling
